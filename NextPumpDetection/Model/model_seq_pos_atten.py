...
class ModelSeqPosAtten(Model):
    def __init__(self, tensor_dict, train_config):
        super(ModelSeqPosAtten, self).__init__(tensor_dict, train_config)
        ...
        self.pos_score_global = tf.get_variable("pos_score", [1, 50, 9], initializer=tf.zeros_initializer())

        pos_score_global = tf.tile(self.pos_score_global, [256, 1, 1])
        paddings = tf.ones_like(pos_score_global) * (-2 ** 32 + 1)

        pos_score = tf.where(tf.logical_not(mask_2d), paddings, pos_score_global)
        self.pos_score = tf.nn.softmax(pos_score, axis=1)
        self.opt_seq_embedding_mean = tf.reduce_sum(self.pos_score * self.masked_opt_seq_embedding, axis=1)
        ...
        self.opt_seq_coin_embedding_mean = self.opt_seq_coin_embedding_sum / tf.cast(tf.expand_dims(new_length, axis=1),
                                                                             tf.float32)
    ...
    def build(self):
        ...
        self.inp = tf.concat([self.target_features, self.opt_seq_embedding_mean, self.opt_seq_coin_embedding_mean], axis=1)
